...
import { Search, Plus, Eye, Edit, Trash2, CheckCircle, Globe, XCircle, Calendar, Users, Trophy, Megaphone, FileText, RefreshCw, PlusCircle, Hash } from "lucide-react";
...
const Button = ({ children, onClick, disabled, className = '', small = false, type = 'primary', isSubmit = false }) => {
    ...
    return <button type={isSubmit ? "submit" : "button"} onClick={(e) => { e.stopPropagation(); onClick && onClick(); }} disabled={disabled} className={`${baseStyle} ${sizeStyle} ${typeStyle} ${className}`}>{children}</button>;
};
...
export default function AdminDashboard() {
    ...
    const handleSetPotd = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.message || "Failed to set Problem of the Day");
            }
    ...
    const toggleAnnouncementGlobalStatus = async (id, currentStatus) => {
        try {
            ...
            toast.success(
                `Announcement ${currentStatus ? "removed from" : "added to"} global announcements!`
            );
        } catch (error) {
    ...
    const toggleProblemGlobalStatus = async (id, currentStatus) => {
        try {
            ...
            toast.success(
                `Problem ${currentStatus ? "removed from" : "added to"} global problems!`
            );
        } catch (error) {
            console.error("Error updating global status:", error);
    ...
    const toggleContestGlobalStatus = async (id, currentStatus) => {
        try {
            ...
            toast.success(
                `Contest ${currentStatus ? "removed from" : "added to"} global contests!`
            );
        } catch (error) {
            console.error("Error updating global status:", error);
        ...
        <div className={`min-h-screen ${retroThemeColors.bgPrimary} font-retro`}>
            ...
            <main className="max-w-7xl mx-auto p-4 sm:p-6">
                <div className="flex flex-wrap gap-2 mb-8">
                    ...
                    <TabButton isActive={activeTab === 'potd'} onClick={() => handleTabClick('potd')}>POTD</TabButton> 
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <RetroCard className="p-6">
                            <div className="flex justify-between items-start mb-4">
                                ...
                                >
                                    <RefreshCw className={`w-4 h-4 ${isPotdLoading ? 'animate-spin' : ''}`} />
                                </Button>
                            </div>
                            
                            {isPotdLoading ? (
                                <div className="flex justify-center items-center h-32">
                                    <div className="animate-pulse text-stone-500">Loading POTD...</div>
                                </div>
                            ) : currentPotd ? (
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between p-3 bg-amber-50 border-2 border-stone-800">
                                        <div>
                                            ...
                                            <p className="text-sm text-stone-500">ID: {currentPotd._id}</p>
                                        </div>
                                        <span className={`px-2 py-1 text-xs font-bold border-2 ${retroThemeColors.panelBorder} ${retroThemeColors.difficulty[currentPotd.difficulty] || 'bg-stone-200'}`}>
                                    ...
                                    <div className="mt-4 flex gap-2">
                                        <Button 
                                            onClick={() => navigate(`/admin/problems/${currentPotd.slug}`)}
                                            className="flex-1"
                                        >
                                ...
                                <div className="text-center py-6">
                                    ...
                                    <p className="text-lg text-stone-500">No Problem of the Day is set</p>
                                </div>
                            )}
                        </RetroCard>
                        ...
                        <RetroCard className="p-6">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className={`p-3 ${retroThemeColors.accentBg} border-2 ${retroThemeColors.panelBorder} text-sm`}>
                    ...
                    isLoading ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {filteredItems.map((item) => {
                                if (activeTab === "problems") return <ProblemCard key={item._id} problem={item} onView={() => handleViewProblem(item.slug)} onEdit={() => handleEditProblem(item.slug)} onVerify={() => handleVerifyProblem(item._id, problems, setProblems)} onDelete={() => handleDeleteProblem(item._id, problems, setProblems)} onToggleGlobal={() => toggleProblemGlobalStatus(item._id, item.isGlobal)} />;
                                if (activeTab === "contests") return <ContestCard key={item._id} contest={item} onView={() => handleViewContest(item.slug)} onEdit={() => handleEditContest(item.slug)} onVerify={() => handleVerifyContest(item._id, contests, setContests)} onDelete={() => handleDeleteContest(item._id, contests, setContests)} onToggleGlobal={() => toggleContestGlobalStatus(item._id, item.isGlobal)} />;
                                if (activeTab === "announcements") return <AnnouncementCard key={item._id} announcement={item} onView={() => handleViewAnnouncement(item._id)} onEdit={() => handleEditAnnouncement(item._id)} onVerify={() => handleVerifyAnnouncement(item._id, announcements, setAnnouncements)} onDelete={() => handleDeleteAnnouncement(item._id, announcements, setAnnouncements)} onToggleGlobal={() => toggleAnnouncementGlobalStatus(item._id, item.isGlobal)} />;
                                return null;
                            })}
                        </div>
                    ) : (
