...
const TeamMemberImage = ({ member, onHover }) => (
  ...
  >
    ...
    />
    <div
      className={`absolute -bottom-2 left-1/2 -translate-x-1/2 w-3/4 h-3 ${retroThemeColors.textAccent} transition-all duration-300 opacity-0 group-hover:opacity-100`}
    ></div>
  </motion.div>
);
...
const ContactButton = ({ href, icon, text, color }) => (
  ...
  >
    <div
      className={`w-8 h-8 flex items-center justify-center border-2 ${retroThemeColors.panelBorder} ${color.iconBg} ${color.iconText}`}
    >
      {icon}
    </div>
...
const ValueCard = ({ icon, title, children }) => (
  <div
    className={`p-6 border-2 ${retroThemeColors.panelBorder} bg-stone-50 h-full`}
  >
    <div className="flex items-center gap-4 mb-3">
      <div
        className={`w-12 h-12 flex-shrink-0 flex items-center justify-center border-2 ${retroThemeColors.panelBorder} ${retroThemeColors.decorativePanelBg} ${retroThemeColors.textAccent}`}
      >
        {icon}
      </div>
...
export default function AboutUs() {
  ...
  return (
    <div
      className={`min-h-screen ${retroThemeColors.bgPrimary} px-4 py-16 font-retro`}
    >
      <div className="max-w-6xl mx-auto">
            ...
            <RetroCard className="p-8 min-h-[24rem] flex flex-col justify-center">
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <p
                    className={`text-lg ${retroThemeColors.textSecondary} mb-4`}
                  >
                    {activeMember.role}
                  </p>
        ...
        <RetroCard className="p-8 mb-16">
          <h2
            className={`text-4xl font-bold text-center mb-8 ${retroThemeColors.textPrimary}`}
          >
            Our Values
          </h2>
        ...
        <RetroCard className="p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-4">
              ...
              <ContactButton
                href="mailto:decodebycode.pvt.ltd@gmail.com"
                icon={<Mail />}
                text="Email Us"
                color={{
                  bg: "bg-rose-100",
