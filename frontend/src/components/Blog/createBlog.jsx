...
const LoadingState = ({ text }) => (
  <div
    className={`min-h-screen flex items-center justify-center ${retroThemeColors.bgPrimary} font-retro`}
  >
    <div className="flex flex-col items-center">
      <div
        className={`w-16 h-16 border-4 ${retroThemeColors.panelBorder} border-t-transparent rounded-full animate-spin`}
      ></div>
      <p
        className={`mt-4 text-2xl animate-pulse ${retroThemeColors.textPrimary}`}
      >
        {text}
      </p>
    </div>
...
const AuthRequiredState = ({ blogId }) => {
  ...
  return (
    <div
      className={`min-h-screen ${retroThemeColors.bgPrimary} flex items-center justify-center p-4 font-retro`}
    >
      <RetroCard className="p-8 text-center max-w-md">
        <h2 className={`text-3xl mb-3 ${retroThemeColors.textAccent}`}>
          Authentication Required
        </h2>
        <p className={`text-xl mb-6 ${retroThemeColors.textSecondary}`}>
          You need to be logged in to {blogId ? "edit" : "create"} a blog post.
        </p>
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Button
            onClick={() => navigate("/login")}
            className={`${retroThemeColors.buttonPrimaryBg} ${retroThemeColors.buttonText}`}
          >
            Log In
          </Button>
...
export default function CreateBlogForm({ blogId }) {
  ...
  // --- Core Logic (Functionality Unchanged) ---
  useEffect(() => {
  ...
  return (
    <div
      className={`min-h-screen ${retroThemeColors.bgPrimary} font-retro p-4 sm:p-6 lg:p-8`}
    >
      <div className="max-w-5xl mx-auto">
        <RetroCard className="p-6 mb-8">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className={`text-lg mt-1 ${retroThemeColors.textSecondary}`}>
        ...
        <form onSubmit={handleSubmit}>
          <RetroCard className="p-8">
            {error && (
              <div
                className={`p-4 mb-6 border-2 ${retroThemeColors.panelBorder} ${retroThemeColors.errorBg} ${retroThemeColors.errorText}`}
              >
                {error}
              </div>
            )}
            ...
            <div className="space-y-8">
              ...
              <FormTextarea
                ...
                onChange={handleChange}
                placeholder="A short, catchy summary for the blog list."
              />
              <FormInput
                label="Thumbnail URL"
                name="thumbnailUrl"
              ...
              <div>
                ...
                <div className={`border-2 ${retroThemeColors.panelBorder}`}>
                  <Editor
                    apiKey="xfy31sy8ugnbx53as6d1eturl2idrzmpe9sujc9pixrpqpqu" // Replace with your TinyMCE API key
                    ...
                    init={{
                      ...
                      toolbar:
                        "undo redo | blocks | bold italic underline | bullist numlist | link codesample emoticons",
                      skin: "oxide",
                      content_css: "default",
                      content_style: `
              ...
              <div
                className={`p-6 border-2 border-dashed ${retroThemeColors.panelBorder} ${retroThemeColors.infoBg} space-y-6`}
              >
